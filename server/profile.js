...
function rewriteImageUrls(obj, nodeBaseUrl) {
  ...
  for (const key of Object.keys(obj)) {
    const val = obj[key];
    if (typeof val === "string" && (key === "image" || key === "image_url" || key.endsWith("_url"))) {
      const pyStaticMatch = val.match(/^https?:\/\/[^/]+:\d+(\/static\/.+)$/);
      if (pyStaticMatch) { obj[key] = `${nodeBaseUrl}${pyStaticMatch[1]}`; continue; }
      if (val.startsWith("/") || (val.length > 2 && val[1] === ":")) {
        const normalized = val.replace(/\\/g, "/");
        const idx = normalized.indexOf("/outfit_model/");
        if (idx !== -1) {
          obj[key] = `${nodeBaseUrl}/static/${normalized.substring(idx + "/outfit_model/".length)}`;
          continue;
        }
      }
...
function formDataToBuffer(form) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    form.on("data", chunk => chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk, "binary")));
...
const bucketFor = (cat) => {
  ...
  if (["tshirt", "shirt", "top", "dress", "sweater", "jacket", "coat", "hoodie", "blouse"].includes(c)) return "tshirts";
  if (["pants", "jeans", "trouser", "trousers", "shorts", "skirt"].includes(c)) return "jeans";
  if (["shoes", "shoe", "sneaker", "sneakers", "boot", "boots"].includes(c)) return "shoes";
...
router.post("/upload-wardrobe", upload.single("image"), async (req, res) => {
  try {
    ...
    const response = await fetchWithRetry(`${OUTFIT_API_URL}/upload-outfit`, { method: "POST", body, headers });
...
router.get("/job/:jobId", async (req, res) => {
  try {
    ...
    const response = await fetchWithRetry(`${OUTFIT_API_URL}/job/${encodeURIComponent(jobId)}`);
    const result = await response.json();
...
router.get("/wardrobe/:userId", async (req, res) => {
  try {
    ...
    const wardrobe = { tshirts: [], jeans: [], shoes: [], watches: [], caps: [], bags: [] };
    ...
    for (const row of items) {
      ...
      if (finalImage) { attrs.image = finalImage; attrs.image_url = imageUrl; }
...
router.delete("/wardrobe/:userId/item/:itemId", async (req, res) => {
  try {
    ...
    const imageUrl = existing.image_url ||
      (existing.attributes && existing.attributes.image) || null;
    ...
    if (deleteErr) {
      ...
      return res.status(500).json({ success: false, error: deleteErr.message || String(deleteErr) });
    }
    ...
    if (fs.existsSync(vectorPath)) {
      try {
        ...
        const filtered = filename
          ? vectors.filter(v => !String(v.image_path || "").replace(/\\/g, "/").endsWith(filename))
          : vectors;
...
router.get("/segmented/:userId", (req, res) => {
  try {
    const { userId } = req.params;
    if (!/^[\w-]+$/.test(userId)) return res.status(400).json({ error: "invalid user id" });

    const segDir = path.join(OUTFIT_MODEL_DIR, "uploads", `${userId}_segmented`);
    if (!fs.existsSync(segDir)) return res.json({ success: true, items: [] });

    const files = fs.readdirSync(segDir).filter(f => /\.(png|jpe?g|webp)$/i.test(f));
    const nodeBase = `${req.protocol}://${req.get("host")}`;
...
router.get("/wardrobe/:userId/summary", async (req, res) => {
  try {
    const { userId } = req.params;
    const response = await fetchWithRetry(`${OUTFIT_API_URL}/wardrobe/${encodeURIComponent(userId)}/summary`);
    const result = await response.json();
    res.status(response.status).json(result);
  } catch (err) {
