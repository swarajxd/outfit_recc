  ...
  const handleRunPipeline = async () => {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        // For native (iOS/Android): use the RN-style object
      ...
      const userId =
        user?.id || user?.emailAddresses?.[0]?.emailAddress || "default_user";
      ...
      const uploadData = await Promise.race([
        new Promise<any>((resolve, reject) => {
          ...
          xhr.onerror = () => {
            const errorMsg =
              `Upload failed. Server at ${API_BASE_URL} is not responding.\n\n` +
              `Possible issues:\n` +
          ...
          xhr.upload.onprogress = (e) => {
            if (e.lengthComputable) {
              ...
              setResultMessage(
                `🔄 Uploading image... ${Math.round(percentComplete)}% (${Math.round(elapsed)}s)`,
              );
          ...
          const progressCheckInterval = setInterval(() => {
            ...
            if (timeSinceLastProgress > 30000) {
              ...
              reject(
                new Error(
                  "Upload stalled. Network may be too slow. Try a smaller image or better WiFi.",
                ),
              );
            }
          }, 5000);
        ...
        new Promise<never>((_, reject) => {
          setTimeout(() => {
            reject(
              new Error(
                "Upload timeout after 2 minutes. Image may be too large or network too slow.",
              ),
            );
          }, 120000); // 2 minutes
        }),
      ...
      while (attempts < maxAttempts) {
        ...
        const statusData = await new Promise<any>((resolve, reject) => {
          ...
          xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) {
              ...
              try {
                const errorData = JSON.parse(xhr.responseText);
                reject(
                  new Error(errorData.error || `Job failed: ${xhr.status}`),
                );
              } catch {
          ...
          xhr.onerror = () =>
            reject(new Error("Network error checking status"));
        ...
        >
          ...
          >
            ...
            <Text style={styles.description}>
              Capture your style and let AI analyze your outfit with advanced
              YOLO detection
            </Text>
