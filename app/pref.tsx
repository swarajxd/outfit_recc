...
function Shell({
  ...
  const continueScale = useRef(
    new Animated.Value(canContinue ? 1 : 0.96),
  ).current;
  const continueOpacity = useRef(
    new Animated.Value(canContinue ? 1 : 0.4),
  ).current;
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(continueScale, {
        toValue: canContinue ? 1 : 0.96,
        duration: 200,
        useNativeDriver: true,
      }),
      Animated.timing(continueOpacity, {
        toValue: canContinue ? 1 : 0.4,
        duration: 200,
        useNativeDriver: true,
      }),
    ]).start();
  ...
  return (
    <View style={ss.shell}>
      ...
      />
      <Animated.View
        style={[ss.blob1, { transform: [{ translateY: blob1Y }] }]}
      >
        <LinearGradient
          colors={["rgba(232,98,10,0.22)", "transparent"]}
          style={{ width: 300, height: 300, borderRadius: 150 }}
        />
      </Animated.View>
      <Animated.View
        style={[ss.blob2, { transform: [{ translateY: blob2Y }] }]}
      >
        <LinearGradient
          colors={["rgba(200,70,10,0.14)", "transparent"]}
        ...
        {!hideProgress && (
          <View style={ss.progressSection}>
            <View style={ss.progressLabels}>
              <Text style={ss.stepLabel}>
                STEP {String(step).padStart(2, "0")} OF {total}
              </Text>
              <Text style={ss.pctLabel}>
          ...
          >
            <BlurView intensity={25} tint="dark" style={ss.continueBtn}>
              <LinearGradient
                colors={
                  canContinue
                    ? ["rgba(50,50,50,0.95)", "rgba(22,22,22,0.98)"]
                    : ["rgba(28,28,28,0.9)", "rgba(18,18,18,0.9)"]
                }
                style={StyleSheet.absoluteFill}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
...
function WelcomeScreen({ onStart }: { onStart: () => void }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setWordIdx((i) => (i + 1) % FASHION_WORDS.length);
    }, 1800);
  ...
  return (
    <View style={ss.shell}>
      ...
      >
        <LinearGradient
          colors={["rgba(232,98,10,0.18)", "transparent"]}
          style={StyleSheet.absoluteFill}
        />
      </View>
      <View
        style={[
          ss.decorCircle,
          { bottom: 40, left: -80, width: 280, height: 280, borderRadius: 140 },
        ]}
      >
        <LinearGradient
          colors={["rgba(180,50,10,0.12)", "transparent"]}
          style={StyleSheet.absoluteFill}
        />
      </View>
      <View style={ss.gridOverlay} pointerEvents="none">
        {[...Array(6)].map((_, i) => (
          <View key={i} style={[ss.gridLine, { top: (H / 6) * i }]} />
        ))}
      </View>
      <SafeAreaView
        style={[
      ...
      >
        ...
        <Animated.Text style={[ss.termsText, { opacity: subAnim }]}>
...
function GenderScreen({ step, total, onBack, onSkip, onContinue }: StepProps) {
  ...
  const selScales = useRef(
    GENDERS.reduce(
      (acc, g) => {
        acc[g.id] = new Animated.Value(1);
        return acc;
      },
  ...
  const select = (id: string) => {
    set("gender", id);
    Animated.sequence([
      Animated.timing(selScales[id], {
        toValue: 0.97,
        duration: 80,
        useNativeDriver: true,
      }),
      Animated.spring(selScales[id], {
        toValue: 1,
        friction: 5,
        tension: 120,
        useNativeDriver: true,
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"How do you express\nyour style?"}
          sub="Select the aesthetic direction that feels most authentic to you."
        />
        <View style={{ gap: 12 }}>
          {GENDERS.map((g, i) => {
                  ...
                  <View style={[ss.pillCard, isSel && ss.pillCardSel]}>
                    ...
                    <View style={[ss.pillIcon, isSel && ss.pillIconSel]}>
                      <MaterialCommunityIcons
                        name={g.icon as any}
                        size={20}
                        color={isSel ? ORANGE : "#777"}
                      />
                    </View>
                    <Text style={[ss.pillLabel, isSel && ss.pillLabelSel]}>
                      {g.label}
                    </Text>
                    {isSel ? (
...
const STYLES = [
  {
    id: "streetwear",
    label: "Streetwear",
    image: require("../assets/streetwear.jpg"),
    colors: ["#1A1A1A", "#2A2A2A"],
  },
  {
    id: "minimal",
    label: "Minimal",
    image: require("../assets/minimal.jpg"),
    colors: ["#202020", "#303030"],
  },
  {
    id: "old_money",
    label: "Old Money",
    image: require("../assets/oldmoney.jpg"),
    colors: ["#2B2118", "#3A2C20"],
  },
  {
    id: "korean",
    label: "Korean Fashion",
    image: require("../assets/korean.jpg"),
    colors: ["#1C1C1C", "#2C2C2C"],
  },
  {
    id: "y2k",
    label: "Y2K",
    image: require("../assets/y2k.jpg"),
    colors: ["#181818", "#2A2A2A"],
  },
  {
    id: "luxury",
    label: "Luxury",
    image: require("../assets/luxury.jpg"),
    colors: ["#251A12", "#3A281B"],
  },
  {
    id: "casual",
    label: "Casual",
  ...
  },
  {
    id: "sporty",
    label: "Sporty",
    image: require("../assets/sporty.jpg"),
    colors: ["#1A1A1A", "#2F2F2F"],
  },
  {
    id: "smart_casual",
    label: "Smart Casual",
    image: require("../assets/smartcasual.jpg"),
    colors: ["#1F1B18", "#2C2622"],
  },
  {
    id: "dark_academia",
    label: "Dark Academia",
    image: require("../assets/darkacademia.jpg"),
    colors: ["#1B1714", "#2B2420"],
  },
  {
    id: "techwear",
    label: "Techwear",
    image: require("../assets/techwear.jpg"),
    colors: ["#121212", "#222222"],
  },
  {
    id: "oversized",
    label: "Oversized",
    image: require("../assets/oversized.jpg"),
    colors: ["#181818", "#282828"],
  },
];
...
function StylesScreen({ step, total, onBack, onSkip, onContinue }: StepProps) {
  ...
  return (
    ...
    >
      <ScreenTitle
        title={"What's your\nfashion vibe?"}
        sub="Pick all the styles that speak to you. Multi-select allowed."
        px={20}
      />
      <FlatList
        data={STYLES}
      ...
      >
        <View style={[ss.styleCard, isSel && ss.styleCardSel]}>
          ...
          <View style={{ marginTop: "auto" }}>
            ...
            <Text style={[ss.styleLabel, isSel && { color: "#FFF" }]}>
              {item.label}
            </Text>
          </View>
          {isSel && (
...
function ColorsScreen({
  ...
  title,
  subKey,
}: StepProps & { title: string; subKey: keyof Profile }) {
  const { profile, set } = useContext(ProfileCtx);
  ...
  return (
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        <ScreenTitle
          title={title}
          sub="Tap to select all that apply."
          px={20}
        />
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        style={ss.colorChipWrap}
      >
        <View
          style={[
            ss.colorChip,
            { backgroundColor: color.hex },
            isSel && ss.colorChipSel,
          ]}
        >
          {isSel && (
            <Ionicons
              name="checkmark"
              size={16}
              color={isDark(color.hex) ? "#FFF" : "#000"}
            />
          )}
        </View>
        <Text
          style={[ss.colorLabel, isSel && { color: "#FFF" }]}
          numberOfLines={1}
        >
          {color.label}
        </Text>
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"What's your\npreferred fit?"}
          sub="This helps us match the right silhouette for you."
          px={20}
        />
    ...
    <View style={{ alignItems: "center", gap: 3 }}>
      {widths.map((w, i) => (
        <View
          ...
          style={{
            width: w,
            height: i === 0 ? 10 : i === 2 ? 14 : 10,
            backgroundColor: selected ? ORANGE : "#333",
            borderRadius: 4,
          }}
        />
      ))}
  ...
  return (
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        <ScreenTitle
          title={"Your body\ntype?"}
          sub="Helps us recommend the most flattering styles."
          px={20}
        />
    ...
    >
      ...
      >
        <View style={[ss.bodyCard, isSel && ss.bodyCardSel]}>
          ...
          <Text
            style={[ss.bodyLabel, isSel && { color: "#FFF" }]}
            numberOfLines={2}
          >
            {item.label}
          </Text>
...
function SkinToneScreen({
  ...
  const selectTone = (id: string) => {
    set("skinTone", id);
    Animated.sequence([
      Animated.timing(selectedScale[id], {
        toValue: 1.15,
        duration: 100,
        useNativeDriver: true,
      }),
      Animated.spring(selectedScale[id], {
        toValue: 1,
        friction: 6,
        tension: 100,
        useNativeDriver: true,
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"What's your\nskin tone?"}
          sub="Helps us suggest colors and palettes that complement you."
        />
          ...
          {SKIN_TONES.map((t) => {
            ...
            return (
              <Animated.View
                key={t.id}
                style={{
                  transform: [{ scale: selectedScale[t.id] }],
                  alignItems: "center",
                }}
              >
                <TouchableOpacity
        ...
        {selected && (
          <View style={ss.toneInfo}>
            <Text style={ss.toneInfoTxt}>
              Selected: {SKIN_TONES.find((t) => t.id === selected)?.label}
            </Text>
          </View>
        )}
      </ScrollView>
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"How tall\nare you?"}
          sub="Used for proportional outfit recommendations."
        />
...
const BUDGETS = [
  ...
  },
  {
    ...
    sub: "$600+",
    icon: "💜",
    colors: ["#120a1a", "#1a1026"],
  },
  {
    id: "depends",
    label: "Depends on Item",
    sub: "Varies",
    icon: "🤍",
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"What's your\nshopping budget?"}
          sub="We'll show you options within your comfort zone."
        />
        {BUDGETS.map((b, i) => {
          const isSel = selected === b.id;
          return (
            <Animated.View
              key={b.id}
              style={{
                opacity: budgetAnims[i],
                transform: [
                  {
                    translateY: budgetAnims[i].interpolate({
                      inputRange: [0, 1],
                      outputRange: [24, 0],
                    }),
                  },
            ...
            >
              ...
              >
                <LinearGradient
                  colors={b.colors as any}
                  style={[ss.budgetCard, isSel && ss.budgetCardSel]}
                >
                  {isSel && (
                    <LinearGradient
                      colors={["rgba(232,98,10,0.12)", "transparent"]}
                      style={StyleSheet.absoluteFill}
                    />
                  )}
                  <Text style={ss.budgetEmoji}>{b.icon}</Text>
                  <View style={{ flex: 1 }}>
                    <Text style={[ss.budgetLabel, isSel && { color: "#FFF" }]}>
                      {b.label}
                    </Text>
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"Anything you\nnever want to wear?"}
          sub="We'll remove these from your recommendations."
        />
    ...
    >
      ...
      >
        <View style={[ss.avoidChip, isSel && ss.avoidChipSel]}>
          {isSel && (
            <LinearGradient
              colors={["rgba(200,50,50,0.2)", "rgba(200,50,50,0.08)"]}
              style={StyleSheet.absoluteFill}
            />
          )}
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"Where do you\nwear outfits?"}
          sub="Pick all the occasions you dress for."
          px={20}
        />
    ...
    >
      ...
      >
        <View style={[ss.occasionCard, isSel && ss.occasionCardSel]}>
          ...
          <Text
            style={[ss.occasionLabel, isSel && { color: "#FFF" }]}
            numberOfLines={2}
          >
            {item.label}
          </Text>
...
const GOALS = [
  {
    ...
    desc: "Daily curated looks",
    icon: "👗",
  },
  {
    id: "matching",
    label: "Better Matching",
    desc: "Mix & match pieces",
    icon: "🎨",
  },
  {
    id: "body",
    label: "Dress for My Body",
    desc: "Flattering styles",
    icon: "✨",
  },
  {
    id: "shopping",
    label: "Shopping Recs",
    desc: "Find the right buys",
    icon: "🛍️",
  },
  {
    id: "attractive",
  ...
  {
    id: "expensive",
    label: "Look More Expensive",
    desc: "Elevated aesthetics",
    icon: "👑",
  },
  {
    id: "confidence",
    label: "Build Confidence",
    desc: "Feel great daily",
    icon: "⚡",
  },
  {
    id: "celebrity",
    label: "Celebrity Looks",
    desc: "Inspired by icons",
    icon: "🌟",
  },
];
  ...
  return (
    ...
    >
      ...
      >
        <ScreenTitle
          title={"What are your\nfashion goals?"}
          sub="We'll prioritize features based on your goals."
          px={20}
        />
    ...
    >
      ...
      >
        <View style={[ss.goalCard, isSel && ss.goalCardSel]}>
          {isSel && (
            <LinearGradient
              colors={["rgba(232,98,10,0.12)", "rgba(232,98,10,0.04)"]}
              style={StyleSheet.absoluteFill}
            />
          )}
          ...
          <View style={{ flex: 1 }}>
            <Text style={[ss.goalLabel, isSel && { color: "#FFF" }]}>
              {item.label}
            </Text>
...
function SummaryScreen({
  ...
  const summaryItems = [
    ...
    },
    {
      label: "Styles",
      value:
        profile.styles.length > 0 ? `${profile.styles.length} selected` : "—",
      icon: "✨",
    },
    {
      ...
      value:
        profile.favoriteColors.length > 0
          ? `${profile.favoriteColors.length} colors`
          : "—",
      icon: "🎨",
    },
    {
      label: "Preferred Fit",
      value: profile.fit?.replace("_", " ") || "—",
      icon: "👔",
    },
    ...
    {
      label: "Budget",
      value: profile.budget?.replace("_", " ") || "—",
      icon: "💎",
    },
    {
      label: "Occasions",
      value:
        profile.occasions.length > 0
          ? `${profile.occasions.length} selected`
          : "—",
      icon: "🎯",
    },
    {
      label: "Goals",
      value: profile.goals.length > 0 ? `${profile.goals.length} goals` : "—",
      icon: "🚀",
    },
  ];
  return (
    <View style={ss.shell}>
      <LinearGradient
        colors={["#080808", "#0E0A06", "#080808"]}
        style={StyleSheet.absoluteFill}
      />
      <View style={[ss.blob1, { opacity: 0.6 }]}>
        <LinearGradient
          colors={["rgba(232,98,10,0.3)", "transparent"]}
          style={{ width: 300, height: 300, borderRadius: 150 }}
        />
      </View>
      <SafeAreaView style={ss.safeArea}>
        ...
        >
          <View style={ss.summaryBadge}>
            <Text style={ss.summaryBadgeTxt}>✓ PROFILE COMPLETE</Text>
          </View>
          <Text style={ss.summaryTitle}>Your Style Profile{"\n"}is Ready</Text>
          <Text style={ss.summarySub}>
            We'll now personalize every recommendation for you.
          </Text>
        </Animated.View>
        <ScrollView
          showsVerticalScrollIndicator={false}
  ...
  return (
    ...
    >
      <BlurView intensity={12} tint="dark" style={ss.summaryCard}>
        <LinearGradient
          colors={["rgba(40,40,40,0.6)", "rgba(20,20,20,0.6)"]}
          style={StyleSheet.absoluteFill}
        />
...
export default function SenseAIOnboarding() {
  ...
  // Check if onboarding is already complete - if so, redirect to home
  useEffect(() => {
    const checkOnboarding = async () => {
      ...
      try {
        ...
        console.warn("[pref] Onboarding check failed:", err);
      }
      setCheckedOnboarding(true);
    };
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          if (!resp.ok) {
            console.warn(`[pref] API returned status ${resp.status}`, result);
          } else {
            console.log(
              "[pref] Preferences saved to database for",
              userId,
              result,
            );
          }
          ...
          console.warn(
            "[pref] Could not save preferences to DB (non-fatal):",
            dbErr?.message || dbErr,
          );
        }
      }
  ...
  const screens = [
    ...
    <ColorsScreen
      {...stepProps(4)}
      title={"Your favorite\ncolors?"}
      subKey="favoriteColors"
    />,
    <ColorsScreen
      {...stepProps(5)}
      title={"Colors you\nnever wear?"}
      subKey="dislikedColors"
    />,
