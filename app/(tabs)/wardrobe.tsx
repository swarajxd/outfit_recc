...
export default function WardrobeScreen() {
  ...
  const uploadToWardrobe = async () => {
    ...
    setIsUploading(true);
    setUploadProgress(chosenImagen3 ? "Uploading · AI mannequin mode (~60s)…" : "Uploading · Fast mode…");
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        formData.append("image", { uri: asset.uri, type: "image/jpeg", name: "wardrobe.jpg" } as any);
      }
      formData.append("user_id", userId);
      formData.append("use_imagen", chosenImagen3 ? "true" : "false");

      const uploadResp = await fetch(`${serverBase}/api/profile/upload-wardrobe`, { method: "POST", body: formData });
      if (!uploadResp.ok) throw new Error("Upload failed");
      const { job_id: jobId } = await uploadResp.json();
      if (!jobId) throw new Error("No job_id returned");

      setUploadProgress(chosenImagen3 ? "Generating AI mannequin images…" : "Detecting & segmenting items…");

      let attempts = 0;
      const poll = async (): Promise<void> => {
        attempts++;
        const statusJson = await (await fetch(`${serverBase}/api/profile/job/${encodeURIComponent(jobId)}`)).json();
        if (statusJson.status === "completed") {
          ...
          Alert.alert(
            ...
            isDup ? "These items are already in your wardrobe."
              : chosenImagen3 ? `${added} item(s) added with AI mannequin images (${ai} generated).`
              : `${added} item(s) added in fast mode.`
          );
          fetchWardrobe(); return;
        }
        if (statusJson.status === "error") throw new Error(statusJson.error || "Processing failed");
  ...
  const filtered = activeCategory === 0
    ? items
    : items.filter(item => displayCategory(item.category) === categories[activeCategory]);
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <View style={styles.headerRow}>
          <View style={styles.headerLeft}>
            {selectionMode ? (
              <TouchableOpacity style={styles.cancelBtn} onPress={exitSelectionMode}>
        ...
        {selectionMode ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
            {categories.map((cat, i) => (
              ...
              >
                <Text style={[styles.chipText, activeCategory === i && styles.chipTextActive]}>{cat}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        )}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        {isLoading ? (
          ...
          <View style={styles.centered}>
            ...
            <Text style={styles.emptySubtext}>Tap + to add outfit photos and build your digital wardrobe.</Text>
          </View>
        ) : (
          <>
            {!selectionMode && (
              <Text style={styles.hintText}>Long press an item to select and delete</Text>
            )}
      ...
      {showModePicker && (
        <View style={styles.sheetOverlay}>
          <TouchableOpacity style={StyleSheet.absoluteFill} activeOpacity={1} onPress={() => confirmMode(false)} />
          <BlurView intensity={30} tint="dark" style={StyleSheet.absoluteFill} />
          <View style={styles.modeSheet}>
            <View style={styles.sheetHandle} />
            <Text style={styles.sheetTitle}>How should we process your outfit?</Text>
            <Text style={styles.sheetSubtitle}>Choose a mode. You can pick differently each time.</Text>

            <TouchableOpacity style={[styles.modeCard, styles.modeCardFast]} onPress={() => confirmMode(false)} activeOpacity={0.82}>
              ...
              <Text style={styles.modeCardDesc}>Detects and saves items instantly using the cropped photo. No AI generation — your actual clothing image is used directly.</Text>
            ...
            <TouchableOpacity style={[styles.modeCard, styles.modeCardAI]} onPress={() => confirmMode(true)} activeOpacity={0.82}>
              <View style={styles.modeCardHeader}>
              ...
              <Text style={styles.modeCardDesc}>Gemini Vision analyses your clothing then Imagen3 generates a professional product photo on a clean white mannequin.</Text>
              <View style={styles.modeProRow}>
            ...
            <TouchableOpacity style={styles.sheetCancel} onPress={() => { setShowModePicker(false); modeResolveRef.current?.(false); modeResolveRef.current = null; }}>
      ...
      {isConfigOpen && (
        <View style={styles.ipOverlay}>
          <View style={styles.overlayCard}>
            ...
            <Text style={styles.overlaySubtitle}>Enter the base URL of your laptop (e.g. http://192.168.0.10:4000).</Text>
            <TextInput
              style={styles.overlayInput}
              placeholder="http://192.168.x.x:4000"
              placeholderTextColor="rgba(255,255,255,0.4)"
              autoCapitalize="none" autoCorrect={false}
              value={tempServerBase} onChangeText={setTempServerBase}
            />
            <View style={styles.overlayButtons}>
              <TouchableOpacity style={[styles.overlayButton, styles.overlayButtonSecondary]} onPress={() => setIsConfigOpen(false)}>
...
function WardrobeCard({
  item, selectionMode, selected, onLongPress, onPress,
}: {
  item: WardrobeItem;
  ...
  return (
    <TouchableOpacity onLongPress={onLongPress} onPress={onPress} activeOpacity={0.85} delayLongPress={350}>
...
const styles = StyleSheet.create({
  ...
  header: {
    backgroundColor: "rgba(0,0,0,0.88)", paddingHorizontal: 20, paddingTop: 12, paddingBottom: 8,
  ...
  cancelBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
  cancelBtnText: { color: "#fff", fontSize: 13, fontWeight: "600" },
  selectAllBtn: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
  selectAllText: { color: "rgba(255,255,255,0.8)", fontSize: 12, fontWeight: "600" },
  deleteBtn: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 12, backgroundColor: DANGER, minWidth: 90, alignItems: "center" },
  ...
  chip: { paddingHorizontal: 20, paddingVertical: 8, borderRadius: 999, backgroundColor: SOFT_GREY, borderWidth: 1, borderColor: "rgba(255,255,255,0.05)" },
  ...
  hintText: { color: "rgba(255,255,255,0.2)", fontSize: 11, textAlign: "center", marginBottom: 14 },
  grid: { flexDirection: "row", gap: 14 },
  column: { flex: 1, gap: 16 },

  // Card
  card: { backgroundColor: CHARCOAL, borderRadius: 20, overflow: "hidden", borderWidth: 2, borderColor: "transparent" },
  ...
  dimOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.5)", borderRadius: 12 },
  ...
  sheetCancel: { alignItems: "center", paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", marginTop: 2 },
  sheetCancelText: { color: "rgba(255,255,255,0.5)", fontSize: 14, fontWeight: "600" },

  ipOverlay: {
    position: "absolute", top: 0, left: 0, right: 0, bottom: 0,
    backgroundColor: "rgba(0,0,0,0.7)", alignItems: "center", justifyContent: "center", paddingHorizontal: 24,
  },
  overlayCard: { width: "100%", borderRadius: 18, backgroundColor: "#111", padding: 18, borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", gap: 12 },
  overlayTitle: { color: "#fff", fontSize: 16, fontWeight: "700" },
  overlaySubtitle: { color: "rgba(255,255,255,0.6)", fontSize: 12 },
  overlayInput: {
    marginTop: 4, borderRadius: 10, borderWidth: 1, borderColor: "rgba(255,255,255,0.2)",
    paddingHorizontal: 10, paddingVertical: 8, color: "#fff", fontSize: 13, backgroundColor: "rgba(0,0,0,0.7)",
  },
