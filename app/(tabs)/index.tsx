          ...
          <View style={styles.actions}>
            ...
            >
              <Text
                style={{
                  fontSize: 20,
                  color: saved ? PRIMARY : "rgba(255,255,255,0.5)",
                }}
              >
                🔖
              </Text>
...
export default function HomeScreen() {
  ...
  async function fetchPosts() {
    ...
    try {
      ...
      if (!resp.ok)
        throw new Error((json as any)?.error || "Failed to load feed");
      ...
      const normalized: Post[] = (data ?? []).map((p: any) => ({
        ...
        score:
          typeof p.score === "number" ? Math.round(p.score * 100) : undefined,
        liked: p.liked === true,
          ...
          {loading ? (
            ...
            posts.map((p: any) => {
              ...
              const derivedItem = {
                ...
                username:
                  owner.username ||
                  (p.owner_clerk_id === user?.id ? currentUserName : "Unknown"),
                avatar:
